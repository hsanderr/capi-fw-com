...
esp_err_t app_web_server__init(void)
{
    ...
    httpd_handle_t httpd_handle = NULL;                   ///< HTTP daemon handler
...
static esp_err_t app_web_server__post_main_handler(httpd_req_t *req)
{
    ...
    else
    {
        ...
        {
            auhtorized_mac_byte_as_str[0] = request_content[0];
            auhtorized_mac_byte_as_str[1] = request_content[1];
            authorized_mac[i] = strtol(auhtorized_mac_byte_as_str, NULL, 16);
        }
